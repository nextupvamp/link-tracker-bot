...
create table if not exists chat_links (
    ...
    link_id bigint not null primary key references link (id) on delete cascade
);
...
create table if not exists subscriber(
  chat_id bigint not null primary key references chat (id) on delete cascade,
  subscription varchar not null primary key references subscription (url) on delete cascade
);
